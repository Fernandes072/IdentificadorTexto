...
def separar_letras(pasta_origem):
    ...
    for arquivo in arquivos:
        ...
        if os.path.exists('letras'): # Apaga a pasta letras se ela existir
            shutil.rmtree('letras')
        os.makedirs('letras') # Cria a pasta letras
        for retangulo in regiao_letras: 
            ...
            if imagem_letra.shape[0] > 0 and imagem_letra.shape[1] > 0: # Verifica se a imagem da letra é válida, pra não dar erro ao salvar
                cv2.imwrite(f'letras/{nome_arquivo}_{i:04d}.png', imagem_letra) # Salva a letra
